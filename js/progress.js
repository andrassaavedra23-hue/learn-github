 ...
 * 
 * NOTA: La función se llama saveModuleProgress (no completeModule)
 * para evitar conflictos de shadowing con funciones locales en los módulos.
 */
...
/**
 * ⚠️ FUNCIÓN RENOMBRADA para evitar shadowing
 * Antes se llamaba completeModule() pero causaba recursión infinita
 * cuando los módulos HTML tenían una función local con el mismo nombre.
 * 
 * Guarda el progreso de un módulo completado
 * @param {string} moduleId - ID del módulo (ej: 'intro', 'flow', etc.)
 * @param {number} xpEarned - XP ganado en el módulo
 * @returns {boolean} true si se guardó correctamente
 */
function saveModuleProgress(moduleId, xpEarned) {
  console.log(`[progress.js] Guardando módulo: ${moduleId} | XP: ${xpEarned}`);
  
  const session = getSession();
  if (!session) {
    console.warn('[progress.js] No hay sesión activa');
    return false;
  }
  ...
  if (!user) {
    console.warn('[progress.js] Usuario no encontrado');
    return false;
  }
  ...
  // Agregar módulo si no está ya completado
  if (!user.completedModules.includes(moduleId)) {
    user.completedModules.push(moduleId);
    console.log(`[progress.js] Módulo ${moduleId} agregado a completados`);
  }
  
  // Actualizar último módulo visitado
  ...
  console.log(`[progress.js] Progreso guardado exitosamente. XP total: ${user.xp}`);
  return true;
}

/**
 * Agregar XP al usuario (sin completar módulo)
 */
